...
WHERE `update`.`projectId` in (SELECT projectId from follows where userId = :userId)
UNION ALL
SELECT 
...
WHERE `organization_update`.`organizationId` in (SELECT organizationId from organization_follows where userId = :userId)
